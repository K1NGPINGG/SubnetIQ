...
async def start_scan(
    ...
    current_user: User = Depends(require_permission('discovery', 'create')),
...
async def run_discovery(
    ...
    elif scan_type_upper in ("PING", "FULL"):


        from app.tasks.worker import celery_app


        with_snmp = scan_type_upper == "FULL"


        task = celery_app.send_task(


            "tasks.run_ping_asset_discovery",


            args=[str(tenant_id), target_ips, scan_id, with_snmp, snmp_community],
...
@router.post("/{scan_id}/run-now", response_model=DiscoveryScanResponse, summary="Run a scan immediately")
...
async def run_scan_now(
    ...
    """Trigger an immediate execution of a scheduled or pending scan."""
...
async def cancel_scan(
    ...
    current_user: User = Depends(require_permission('discovery', 'manage')),
...
async def delete_scan(
    ...
    current_user: User = Depends(require_permission('discovery', 'delete')),
